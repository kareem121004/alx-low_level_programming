...
int main(int argc, char *argv[])
{
	...
	if (argc != 3)
		dprintf(2, ERROR_ARG_COUNT, argv[0]), exit(97);

	file_from = open(argv[1], O_RDONLY);

	if (file_from == -1)
		dprintf(2, ERROR_READ_FILE , argv[1]), exit(98);
	file_to = open(argv[2], O_WRONLY | O_CREAT | O_TRUNC, 0664);

	if (file_to == -1)
		dprintf(2, ERROR_WRITE_FILE, argv[2]), exit(99);
	...
	if (file_from)
		dprintf(2, ERROR_CLOSE_FD, file_from), exit(100);
	if (file_to)
		dprintf(2, ERROR_CLOSE_FD, file_to), exit(100);
